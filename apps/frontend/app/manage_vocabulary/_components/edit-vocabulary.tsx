...
export default function EditVocabulary({ selectedVocabulary, onEdited }: Props) {
  ...
  const handleOpen = () => {
    ...
    setEntries(
      selectedVocabulary.vocabularyWords.map((word, i) => ({
        word,
        meaning: selectedVocabulary.vocabularyMeanings[i] ?? '',
      }))
    );
    setIsOpen(true);
  };

  const handleChange = (index: number, field: 'word' | 'meaning', value: string) => {
    setEntries(prev =>
  ...
  const handleCommit = async () => {
    ...
    try {
      const res = await fetch('http://localhost:4000/vocabularies/update-entries', {
        method: 'POST',
      ...
      {isOpen && selectedVocabulary && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl max-h-[80vh] flex flex-col gap-4">
            <h2 className="text-xl font-bold">Edit: {selectedVocabulary.vocabularyName}</h2>

            <div className="overflow-y-auto flex-1">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                <div className="font-semibold text-gray-500 text-sm pb-1">Word</div>
                <div className="font-semibold text-gray-500 text-sm pb-1">Meaning</div>

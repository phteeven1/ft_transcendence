...
export class WordBuildingPuzzleEngine implements IWordBuildingPuzzleEngine {
  ...
  private prepareEntries(rawEntries: Array<{ word: string; clue: string }>): {
    ...
    for (const entry of rawEntries) {
      ...
      if (
        ...
        normalized.length > this.GRID_SIZE
      ) {
    ...
    return {
      sortedEntries: this.sortEntriesStrategically(validEntries),
      rejectedWords,
    };
  }
  ...
  private sortEntriesStrategically(
    ...
    return entries.sort((a, b) => {
      ...
      const commonA = a.word
        .split('')
        .filter((l) => (freq.get(l) ?? 0) >= 2).length;
      const commonB = b.word
        .split('')
        .filter((l) => (freq.get(l) ?? 0) >= 2).length;
  ...
  private buildResult(
    ...
    const unplacedWords = [
      ...rejectedWords,
      ...validEntries
        .filter((e) => !context.placedWordsSet.has(e.word))
  ...
  private canPlace(
    ...
    if (preRow >= 0 && preCol >= 0 && grid[preRow][preCol] !== null)
      return false;
  ...
  private scorePlacement(
    ...
    let score = 0;
    score +=
      this.countFutureIntersections(context, candidate, allEntries) *
      FUTURE_INTERSECTION_WEIGHT;

    const dist = this.distanceFromCenter(candidate.row, candidate.col);
    const maxDist = (context.gridSize / 2) * Math.SQRT2;
    score += Math.max(
      0,
      MAX_CENTER_BONUS - (dist / maxDist) * MAX_CENTER_BONUS,
    );
  ...
  private countFutureIntersections(
    ...
    for (const entry of allEntries) {
      if (entry.word === word || context.placedWordsSet.has(entry.word))
        continue;

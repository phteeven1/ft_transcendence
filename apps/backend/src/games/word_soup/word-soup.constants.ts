import {
  estimateIntroDurationMs as estimateSharedIntroDurationMs,
  WORD_SOUP_INTRO_TEXTS,
} from '../intro-sync';
...
/** Wall-clock intro length for Word Soup (shared estimator + Soup texts). */
export function estimateIntroDurationMs(solutionWords: string[]): number {
  return estimateSharedIntroDurationMs(WORD_SOUP_INTRO_TEXTS, solutionWords);
}
